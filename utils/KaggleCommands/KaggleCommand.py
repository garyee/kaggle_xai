    ...
    def execute(self):
        if self.entity == KaggleEntityType.DATASET:
            if (self.operation == KaggleCommandOperations.LIST):
                ...
                checkAndDeleteProperty(self._parameters, 'page-size')
                checkAndDeleteProperty(self._parameters, 'sort-by')
                checkAndDeleteProperty(self._parameters, 'file-type')
                checkAndDeleteProperty(self._parameters, 'license-name')
                checkAndDeleteProperty(self._parameters, 'tag-ids')
                checkAndDeleteProperty(self._parameters, 'max-size')
                checkAndDeleteProperty(self._parameters, 'min-size')
            ...
            raise CommandTypDoesNotExist(
                str(self.entity)+' '+str(self.operation))
        elif self.entity == KaggleEntityType.KERNEL:
            if (self.operation == KaggleCommandOperations.DOWNLOAD):
                # https://github.com/Kaggle/kaggle-api/blob/15cb3f2490db9e0fad4b7662e3621ad949232510/kaggle/api/kaggle_api_extended.py#L828
                checkAndDeleteProperty(self._parameters, 'sort-by')
                checkAndDeleteProperty(self._parameters, 'size')
                checkAndDeleteProperty(self._parameters, 'file-type')
                checkAndDeleteProperty(self._parameters, 'license-name')
                checkAndDeleteProperty(self._parameters, 'tag-ids')
                checkAndDeleteProperty(self._parameters, 'search')
                checkAndDeleteProperty(self._parameters, 'user')
                checkAndDeleteProperty(self._parameters, 'mine')
                checkAndDeleteProperty(self._parameters, 'page')
                checkAndDeleteProperty(self._parameters, 'max-size')
                checkAndDeleteProperty(self._parameters, 'min-size')
                path = self._parameters['path']
                checkAndDeleteProperty(self._parameters, 'path')

                res = getApi().kernel_pull(self.entityRef.split(
                    "/")[0], self.entityRef.split("/")[1], **self.parameters)
                f = open(path+res['metadata']['slug'] +
                         '.ipynb', "w", encoding="utf-8")
...
def checkAndDeleteProperty(object, property):
